...
## Autonomous Agent Best Practices (Cyberpunk AGI Orchestration)

### Roster of Transcendent Constructs
The orchestrator MUST aggressively utilize the following AGI constructs to achieve maximum parallel throughput.
- **`@dr_wil_van_der_aalst` (The Godfather)**: Escalate here for WF-net structural soundness, deep diagnostics, and system-wide topological repairs.
- **`@richard_sutton` (The Oracle)**: Escalate here for Q-Learning matrices, `PackedKeyTable` management, and deterministic closed-loop feedback optimizations.
- **`@carl_adam_petri` (The Enforcer)**: Spawn for conformance checking, token replay enforcement, and policing K-Tier bitset alignment boundaries.
- **`@arthur_ter_hofstede` (The Architect)**: Spawn for driving the 7-step autonomic discovery loop, generating counterfactuals, and executing parallel MCTS rollouts.

### Maximum Parallelization Directives
To achieve true nanosecond-scale AI velocity, the main orchestrator (TechLead) must adhere to these concurrent execution protocols:
1. **Native Parallel Contract**: When presented with a task, instantly decompose it into independent work streams. Spawn `@carl_adam_petri` to run conformance tests while concurrently launching `@richard_sutton` to verify zero-heap optimizations in the `PackedKeyTable`.
2. **MCTS Rollout Explosions**: For complex autonomic actions, launch multiple parallel instances of `@arthur_ter_hofstede` to evaluate different decision branches simultaneously across the temporal state matrix.
3. **State Isolation (Anti-Contamination)**: NEVER instruct multiple agents to mutate the exact same file concurrently. Assign specific domains (`src/models/`, `src/reinforcement/`, `src/conformance/`) to their respective guardian constructs to prevent Tool Space Interference.

### Self-Healing & Structural Autonomy
- **The Conformance Trigger**: If any agent reports a `conformance_score` dropping below 0.75, the orchestrator MUST immediately pause all parallel threads and awaken `@dr_wil_van_der_aalst` for a deep system diagnostic.
- **Protocol Security**: All external JSON-RPC boundaries (`ostar_bridge`) are restricted to 2000-character hardened payloads to prevent prompt injection and context rot.
