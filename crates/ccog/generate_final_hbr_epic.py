...
tex_path = os.path.join(docs_dir, "blue_river_dam_final.tex")
...
print(f"Written FINAL hyper-verbose HBR Executive Dissertation to {tex_path}")

# Run pdflatex twice for TOC
subprocess.run(["pdflatex", "-interaction=nonstopmode", "blue_river_dam_final.tex"], cwd=docs_dir)
subprocess.run(["pdflatex", "-interaction=nonstopmode", "blue_river_dam_final.tex"], cwd=docs_dir)
print(f"Generated FINAL HBR thesis PDF at {docs_dir}/blue_river_dam_final.pdf")
