...
use ccog::runtime::self_play_adversarial::{CcogAdversary, AdversarialTournament, SearchAdversary};
use ccog::runtime::{ClosedFieldContext, cog8::Instinct};
use anyhow::Result;
...
fn test_adversarial_tournament_discovery() -> Result<()> {
    ...
    assert!(max_loss > 0.0, "Adversary should have found at least one false closure or mismatch");
...
fn test_greedy_search_adversary_false_settle() -> Result<()> {
    ...
    assert!(max_loss >= 10.0, "SearchAdversary should have found the False Settle exploit (loss 10.0)");
