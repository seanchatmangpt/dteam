//! Phase 7+ — `ainst` CLI smoke test.
//!
//! Drives every PRD §14.1 command end-to-end through the actual binary,
//! reading and writing JSON fixtures, asserting exit codes and digest
//! propagation. Skipped on CI when the `cli` feature is off.
...
fn ainst_bin() -> PathBuf {
    // cargo test sets CARGO_BIN_EXE_<name> for `[[bin]]` targets when the
    // bin feature is enabled.
...
#[test]
fn cli_full_pipeline_supply_chain() {
    let dir = std::env::temp_dir().join("ainst-cli-smoke");
...
#[test]
fn cli_rejects_constant_policy() {
    let dir = std::env::temp_dir().join("ainst-cli-reject");
    let _ = fs::remove_dir_all(&dir);
    fs::create_dir_all(&dir).unwrap();
    let p = |s: &str| dir.join(s).to_string_lossy().into_owned();

    // Constant policy: always Ask, regardless of context.
    fs::write(
        p("constant.json"),
        r#"{"rules":[
            ["urn:blake3:a","Ask"],
...
fn cli_verify_replay_detects_tamper() {
    ...
    // Tamper with the manifest: change the name field.
