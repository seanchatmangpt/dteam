...
impl ComponentOption {
    ...
    #[must_use]
    pub fn provides_features(&self) -> &BTreeSet<FeatureId> { &self.provides }
    #[must_use]
    pub fn required_features(&self) -> &BTreeSet<FeatureId> { &self.requires }
    #[must_use]
    pub fn exclusions(&self) -> &BTreeSet<String> { &self.excludes }
    #[must_use]
    pub const fn cost(&self) -> u64 { self.cost }
    #[must_use]
    pub const fn latency_micros(&self) -> u64 { self.latency_micros }
    #[must_use]
    pub const fn reliability_ppm(&self) -> u32 { self.reliability_ppm }
    #[must_use]
    pub const fn complexity(&self) -> u32 { self.complexity }
    #[must_use]
    pub const fn is_reversible(&self) -> bool { self.reversible }
    #[must_use]
    pub fn tags(&self) -> &BTreeSet<String> { &self.tags }
}
...
impl Composition {
    ...
    #[must_use]
    pub fn provides(&self) -> &BTreeSet<FeatureId> { &self.provides }
    #[must_use]
    pub const fn cost(&self) -> u64 { self.cost }
    #[must_use]
    pub const fn latency_micros(&self) -> u64 { self.latency_micros }
...
impl CompositionSpace {
    #[must_use]
    pub fn lawful(&self) -> &[Composition] { &self.lawful }
    #[must_use]
    pub fn pareto(&self) -> &[Composition] { &self.pareto }
    #[must_use]
    pub const fn explored(&self) -> usize { self.explored }
    #[must_use]
    pub const fn refused(&self) -> usize { self.refused }
    #[must_use]
    pub const fn digest(&self) -> Digest { self.digest }
}
...
impl Display for CompositionError {
    fn fmt(&self, formatter: &mut Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            Self::EmptyRequirement => formatter.write_str("composition requires at least one feature"),
            Self::SearchBoundTooLarge(size) => write!(formatter, "search contains {size} components; maximum is 24"),
        }
    }
}
...
impl CombinatorialEngine {
    ...
    pub fn insert(&mut self, component: ComponentOption) -> Result<(), CompositionError> {
        if self.components.contains_key(component.id()) {
            return Err(CompositionError::DuplicateComponent(component.id().to_owned()));
        }
    ...
    #[must_use]
    pub fn components(&self) -> &BTreeMap<String, ComponentOption> { &self.components }

    pub fn explore(&self, request: &CompositionRequest) -> Result<CompositionSpace, CompositionError> {
        ...
        for mask in 1..combinations {
            ...
            let selected = options.iter().enumerate()
        ...
        let pareto = lawful.iter()
            .filter(|candidate| !lawful.iter().any(|other| other.digest != candidate.digest && other.dominates(candidate)))
            .cloned()
            .collect::<Vec<_>>();
        let mut encoder = CanonicalEncoder::new();
        encoder.text("type", "combinatorial-space-v1").u64("explored", explored as u64).u64("refused", refused as u64);
        for composition in &lawful { encoder.field("composition", &composition.digest().0); }
        Ok(CompositionSpace { lawful, pareto, explored, refused, digest: encoder.digest() })
    }
}

fn admit_composition(selected: &[&ComponentOption], request: &CompositionRequest) -> Option<Composition> {
    let ids = selected.iter().map(|item| item.id.clone()).collect::<BTreeSet<_>>();
    if selected.iter().any(|item| item.excludes.iter().any(|excluded| ids.contains(excluded))) {
        return None;
    }
    if selected.iter().any(|item| item.tags.iter().any(|tag| request.forbidden_tags.contains(tag))) {
        return None;
    }
    let provides = selected.iter().flat_map(|item| item.provides.iter().cloned()).collect::<BTreeSet<_>>();
    let requires = selected.iter().flat_map(|item| item.requires.iter().cloned()).collect::<BTreeSet<_>>();
    ...
    let reliability_ppm = selected.iter().map(|item| item.reliability_ppm).min().unwrap_or(0);
    ...
    let mut encoder = CanonicalEncoder::new();
    encoder.text("type", "composition-v1").u64("cost", cost).u64("latency", latency_micros).u64("reliability", u64::from(reliability_ppm)).u64("complexity", u64::from(complexity));
    for component in &components { encoder.text("component", component); }
    Some(Composition { components, provides, cost, latency_micros, reliability_ppm, complexity, reversible, digest: encoder.digest() })
}
...
impl VisionWizard {
    ...
    pub fn standard() -> Self {
        ...
        let entries = [
            ("mode", "What operating mode is primary?", vec!["developer", "edge", "telco", "enterprise"], "Sets the topology and governance envelope."),
            ("availability", "What availability tier is required?", vec!["standard", "ha", "carrier"], "Determines redundancy and reliability constraints."),
            ("authority", "Must every side effect pass through BRCE?", vec!["yes", "no"], "Controls whether actuation can receive standing."),
            ("offline", "Must the system operate offline?", vec!["yes", "no"], "Selects local-first and deterministic transports."),
            ("reversible", "Must every selected option remain reversible?", vec!["yes", "no"], "Preserves lawful alternatives before irreversible selection."),
        ];
        for (id, prompt, choices, rationale) in entries {
            wizard.questions.insert(id.to_owned(), WizardQuestion {
                id: id.to_owned(), prompt: prompt.to_owned(),
                choices: choices.into_iter().map(str::to_owned).collect(),
                required: true, rationale: rationale.to_owned(),
            });
        }
        wizard
    }
    ...
    pub fn answer(&mut self, id: &str, value: WizardValue) -> Result<(), WizardError> {
        let question = self.questions.get(id).ok_or_else(|| WizardError::UnknownQuestion(id.to_owned()))?;
        if let WizardValue::Choice(choice) = &value {
            if !question.choices.is_empty() && !question.choices.contains(choice) {
                return Err(WizardError::InvalidChoice { question: id.to_owned(), choice: choice.clone() });
            }
        }
    ...
    pub fn unanswered(&self) -> Vec<&WizardQuestion> {
        self.questions.values().filter(|question| question.required && !self.answers.contains_key(question.id())).collect()
    }

    pub fn compile(&self) -> Result<WizardPlan, WizardError> {
        let missing = self.unanswered().into_iter().map(|question| question.id.clone()).collect::<Vec<_>>();
        if !missing.is_empty() { return Err(WizardError::MissingAnswers(missing)); }
        let choice = |id: &str| match self.answers.get(id) {
            Some(WizardValue::Choice(value)) => value.as_str(),
            _ => "",
        };
        let mut required = BTreeSet::new();
        for feature in ["identity", "receipts", "doctor"] { required.insert(FeatureId::new(feature).expect("static feature")); }
        if choice("authority") == "yes" { required.insert(FeatureId::new("brce").expect("static feature")); }
        if choice("offline") == "yes" { required.insert(FeatureId::new("offline").expect("static feature")); }
        if choice("mode") == "telco" { required.insert(FeatureId::new("telco").expect("static feature")); }
        ...
        let request = CompositionRequest::new(required)
            .bounds(12, 10_000, if choice("mode") == "edge" { 5_000 } else { 100_000 })
            .minimum_reliability(minimum_reliability)
            .require_reversible(choice("reversible") == "yes")
            .forbid_tag(if choice("offline") == "yes" { "network-required" } else { "never" });
        let commands = vec![
        ...
        for (id, answer) in &self.answers { encoder.text("answer", &format!("{id}:{answer:?}")); }
        Ok(WizardPlan { request, commands, digest: encoder.digest() })
    }

    #[must_use]
    pub fn questions(&self) -> &BTreeMap<String, WizardQuestion> { &self.questions }
}
...
impl WizardPlan {
    #[must_use]
    pub fn request(&self) -> &CompositionRequest { &self.request }
    #[must_use]
...
impl Display for WizardError {
    fn fmt(&self, formatter: &mut Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            Self::InvalidChoice { question, choice } => write!(formatter, "invalid choice `{choice}` for `{question}`"),
...
pub enum TelcoRole { Edge, Access, Transport, Core, Control, Data, Observability }
...
impl TelcoNode {
    #[must_use]
    pub fn new(id: impl Into<String>, role: TelcoRole, domain: FailureDomain, capacity: u64) -> Self {
        Self { id: id.into(), role, domain, capacity }
    }
    #[must_use]
    pub fn id(&self) -> &str { &self.id }
    #[must_use]
    pub const fn role(&self) -> TelcoRole { self.role }
    #[must_use]
    pub fn domain(&self) -> &FailureDomain { &self.domain }
    #[must_use]
...
impl TelcoLink {
    #[must_use]
    pub fn new(from: impl Into<String>, to: impl Into<String>, latency_micros: u64, capacity: u64, reliability_ppm: u32) -> Self {
        Self { from: from.into(), to: to.into(), latency_micros, capacity, reliability_ppm: reliability_ppm.min(1_000_000) }
    }
}
...
impl TelcoPath {
    ...
    pub const fn reliability_ppm(&self) -> u32 { self.reliability_ppm }
    #[must_use]
    pub fn failure_domains(&self) -> &BTreeSet<FailureDomain> { &self.failure_domains }
    #[must_use]
    pub const fn digest(&self) -> Digest { self.digest }
}
...
impl TelcoAssessment {
    #[must_use]
    pub fn compliant_paths(&self) -> &[TelcoPath] { &self.compliant_paths }
    #[must_use]
    pub const fn disjoint_path_count(&self) -> usize { self.disjoint_path_count }
    #[must_use]
    pub fn single_points_of_failure(&self) -> &[String] { &self.single_points_of_failure }
    #[must_use]
    pub const fn standing(&self) -> &'static str { self.standing }
    #[must_use]
    pub const fn digest(&self) -> Digest { self.digest }
}
...
impl TelcoTopology {
    ...
    pub fn add_node(&mut self, node: TelcoNode) { self.nodes.insert(node.id.clone(), node); }
    pub fn add_link(&mut self, link: TelcoLink) { self.links.push(link); }
    ...
    pub fn assess(&self, from: &str, to: &str, objective: &ServiceObjective) -> TelcoAssessment {
        ...
        let mut compliant_paths = all.into_iter().filter(|path| {
        ...
        compliant_paths.sort_by(|a, b| a.latency_micros.cmp(&b.latency_micros).then_with(|| b.reliability_ppm.cmp(&a.reliability_ppm)).then_with(|| a.nodes.cmp(&b.nodes)));
        ...
        let single_points_of_failure = counts.into_iter().filter_map(|(node, count)| (count == compliant_paths.len() && count > 0).then_some(node)).collect::<Vec<_>>();
        let standing = if compliant_paths.is_empty() { "BLOCKED" }
            else if disjoint_path_count >= 2 && single_points_of_failure.is_empty() { "ALIVE" }
            else { "PARTIAL_ALIVE" };
        let mut encoder = CanonicalEncoder::new();
        encoder.text("type", "telco-assessment-v1").text("standing", standing).u64("disjoint", disjoint_path_count as u64);
        for path in &compliant_paths { encoder.field("path", &path.digest.0); }
        TelcoAssessment { compliant_paths, disjoint_path_count, single_points_of_failure, standing, digest: encoder.digest() }
    }

    fn paths(&self, from: &str, to: &str, maximum_hops: usize) -> Vec<TelcoPath> {
        if !self.nodes.contains_key(from) || !self.nodes.contains_key(to) { return Vec::new(); }
        ...
        while let Some((nodes, latency, capacity, reliability)) = queue.pop_front() {
            ...
            if current == to {
                let domains = nodes.iter().filter_map(|id| self.nodes.get(id).map(|node| node.domain.clone())).collect::<BTreeSet<_>>();
                let mut encoder = CanonicalEncoder::new();
                encoder.text("type", "telco-path-v1").u64("latency", latency).u64("capacity", capacity).u64("reliability", u64::from(reliability));
                for node in &nodes { encoder.text("node", node); }
                results.push(TelcoPath { nodes, latency_micros: latency, capacity, reliability_ppm: reliability, failure_domains: domains, digest: encoder.digest() });
                continue;
            }
            if nodes.len() > maximum_hops { continue; }
            for link in self.links.iter().filter(|link| &link.from == current) {
                if nodes.contains(&link.to) { continue; }
                let mut next = nodes.clone(); next.push(link.to.clone());
                queue.push_back((next, latency.saturating_add(link.latency_micros), capacity.min(link.capacity), reliability.min(link.reliability_ppm)));
            }
        }
        results
    }
    ...
    pub fn standard() -> Self {
        ...
        for node in [
            TelcoNode::new("edge-a", TelcoRole::Edge, FailureDomain::new("zone-a"), 10_000),
            TelcoNode::new("edge-b", TelcoRole::Edge, FailureDomain::new("zone-b"), 10_000),
            TelcoNode::new("transport-a", TelcoRole::Transport, FailureDomain::new("metro-a"), 20_000),
            TelcoNode::new("transport-b", TelcoRole::Transport, FailureDomain::new("metro-b"), 20_000),
            TelcoNode::new("core-a", TelcoRole::Core, FailureDomain::new("region-a"), 50_000),
            TelcoNode::new("core-b", TelcoRole::Core, FailureDomain::new("region-b"), 50_000),
...
pub fn standard_combinatorial_engine() -> CombinatorialEngine {
    ...
    let options = [
        ComponentOption::new("local-kernel").unwrap().provides(feature("identity")).provides(feature("receipts")).provides(feature("offline")).economics(10, 200, 2).reliability(999_999).tag("local-first"),
        ComponentOption::new("brce-runtime").unwrap().provides(feature("brce")).requires(feature("receipts")).economics(20, 400, 3).reliability(999_999),
        ComponentOption::new("doctor-plane").unwrap().provides(feature("doctor")).requires(feature("identity")).economics(5, 50, 1).reliability(1_000_000),
        ComponentOption::new("telco-fabric").unwrap().provides(feature("telco")).requires(feature("receipts")).economics(100, 2_000, 5).reliability(999_999).tag("carrier"),
        ComponentOption::new("cloud-control").unwrap().provides(feature("doctor")).provides(feature("telco")).economics(60, 5_000, 4).reliability(999_000).tag("network-required").excludes("local-only"),
        ComponentOption::new("local-only").unwrap().provides(feature("offline")).economics(1, 10, 1).reliability(999_999).excludes("cloud-control"),
        ComponentOption::new("dual-region-replay").unwrap().provides(feature("telco")).requires(feature("receipts")).economics(80, 1_500, 4).reliability(1_000_000).tag("carrier"),
    ];
    for option in options { engine.insert(option).expect("unique standard option"); }
    engine
}
...
mod tests {
    ...
    fn composition_search_preserves_multiple_lawful_options() {
        ...
        let request = CompositionRequest::new([FeatureId::new("identity").unwrap(), FeatureId::new("doctor").unwrap()]).bounds(5, 200, 10_000);
    ...
    fn wizard_compiles_telco_carrier_constraints() {
        ...
        for (id, choice) in [("mode", "telco"), ("availability", "carrier"), ("authority", "yes"), ("offline", "yes"), ("reversible", "yes")] {
            wizard.answer(id, WizardValue::Choice(choice.to_owned())).unwrap();
        }
        let plan = wizard.compile().unwrap();
        let space = standard_combinatorial_engine().explore(plan.request()).unwrap();
    ...
    fn telco_assessment_requires_redundant_compliant_paths() {
        ...
        let objective = ServiceObjective { maximum_latency_micros: 2_000, minimum_capacity: 8_000, minimum_reliability_ppm: 999_999, minimum_failure_domains: 3 };
    ...
    fn impossible_slo_is_blocked() {
        let topology = TelcoTopology::standard();
        let objective = ServiceObjective { maximum_latency_micros: 100, minimum_capacity: 100_000, minimum_reliability_ppm: 1_000_000, minimum_failure_domains: 6 };
        assert_eq!(topology.assess("edge-a", "core-a", &objective).standing(), "BLOCKED");
    }
}
