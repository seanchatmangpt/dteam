...
use dteam::ref_conformance::ref_token_replay::{
    apply_token_based_replay_bcinr, apply_token_based_replay_optimized,
    apply_token_based_replay_standard,
};
use dteam::ref_models::ref_event_log::EventLogActivityProjection;
use dteam::ref_models::ref_petri_net::{ArcType, Marking, PetriNet};
use std::path::Path;
...
fn bench_replay_parity(c: &mut Criterion) {
    ...
    group.bench_function("Standard Replayer", |b| {
        b.iter(|| apply_token_based_replay_standard(black_box(&net), black_box(&projection)))
    });

    // 2. Optimized (bcinr-style pre-computation)
    group.bench_function("BCINR Optimized Replayer", |b| {
        b.iter(|| apply_token_based_replay_optimized(black_box(&net), black_box(&projection)))
    });

    // 3. Pure Bitset (bcinr bitset algebra)
    group.bench_function("BCINR Pure Bitset Replayer", |b| {
